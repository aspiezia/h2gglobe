        ...
        BaseGenLevelSmearer *genSys, BaseSmearer *phoSys, BaseDiPhotonSmearer * diPhoSys)
{
	...
	diphotonVHmet_id = -1; //met at analysis step
        ...
        if(includeVHlepPlusMet){
            ...
            float myweight=1.;
            if(eventweight*sampleweight!=0) myweight=eventweight/sampleweight;
	    VHLepTag2012(l, diphotonVHlep_id, VHlep1event, VHlep2event, false, mu_ind, muVtx, VHmuevent_cat, el_ind, elVtx, VHelevent_cat, &smeared_pho_energy[0], 
			phoidMvaCut, eventweight, smeared_pho_weight, isSyst);
	}
